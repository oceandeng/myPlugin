    ...
    */
    ...
    function pickerDateRange(inputId, options) {
        ...
        var wrapper = {
            ...
            ta:[
                ...
                '<div style="width: 100%;height: 100%;position: fixed;background: rgba(0,0,0,0.6);left: 0;top: 0;"></div>',
                '<div class="ta_calendar_content">',
                    ...
                    '<div class="ta_calendar_footer cf" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
                        '<div class="frm_msg" style="display:none">',
                            '<div id="' + this.dateRangeDiv + '" class="ta_ipt_text_con">',
                                '<input type="text" class="ta_ipt_text_s ta_ipt_start" name="' + this.startDateId + '" id="' + this.startDateId + '" value="' + this.mOpts.startDate + '" readonly />',
                                '<span class="' + this.mOpts.joinLineId + '"> - </span>',
                                '<input type="text" class="ta_ipt_text_s ta_ipt_end" name="' + this.endDateId + '" id="' + this.endDateId + '" value="' + this.mOpts.endDate + '" readonly /><br />',
                            '</div>',
                        ...
                        '</div>',
                        '<div class="frm_btn">',
                            '<a class="ta_btn ta_btn_primary" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" >确定</a>',
                            '<a class="ta_btn" type="button" id="' + this.closeBtn + '">取消</a>',
                        '</div>',
                    '</div>',
                '</div>',
                '</div>'
        ...
        $('#' + this.submitBtn).bind('click', function() {
            ...
            __method.mOpts.success({'startDate': $('#' + __method.mOpts.startDateId).val(), 
                                    ...
                                    'totalPrice':__method.mOpts.totalPrice({'startDate': $('#' + __method.mOpts.startDateId).val(),'endDate': $('#' + __method.mOpts.endDateId).val()})
                                    });
            return false;
        });
        // 日期选择关闭按钮的 click 事件
    ...
    pickerDateRange.prototype.addCSS = function(isCompare, specialClass) {
        ...
        for(var d = new Date(sDate); d.getTime() <= eDate.getTime(); d.setDate(d.getDate() + 1)) {
            ...
            $('#' + this.calendarId + '_' + this.date2ymd(d).join('-')).addClass(cla);
            $('#' + this.calendarId + '_' + this.date2ymd(d).join('-')).addClass(this.calendarId + '_' + this.date2ymd(d).join('-'));
        }
        if(this.mOpts.theme == 'ta'){
            //为开始结束添加特殊样式
            $('#' + this.calendarId + '_' + this.date2ymd(new Date(sDate)).join('-')).addClass(this.mOpts.firstCss);
            $('#' + this.calendarId + '_' + this.date2ymd(new Date(eDate)).join('-')).addClass(this.mOpts.lastCss);
            $('#' + this.calendarId + '_' + this.date2ymd(new Date(sDate)).join('-')).addClass(this.calendarId + '_' + this.date2ymd(new Date(sDate)).join('-'));
            $('#' + this.calendarId + '_' + this.date2ymd(new Date(eDate)).join('-')).addClass(this.calendarId + '_' + this.date2ymd(new Date(eDate)).join('-'));
            //如果开始结束时间相同
    ...
    pickerDateRange.prototype.checkDisableDay = function(startYmd, endYmd){
        var sDate = this.str2date(startYmd);
        var eDate = this.str2date(endYmd);
        var sTime = sDate.getTime();
        var eTime = eDate.getTime();
        ...
        for(var i=0;i<this.mOpts.disCertainYMDay.length;i++){
            var dateTime= new Date(this.mOpts.disCertainYMDay[i]).getTime();
            if(sTime<dateTime && dateTime<eTime){
                this.dateInput = this.startDateId;
                return true;
            }
        }
    ...
    pickerDateRange.prototype.selectDate = function(ymd) {
        ...
        // start <-> end 切换
        if(this.startDateId == this.dateInput || this.checkDisableDay($('#' + this.startDateId).val(), ymd)) {
            tmpBool=false;
            console.info("开始时间：",ymd);
            // 移除样式
            this.removeCSS(0);
            this.removeCSS(1);
            // 为当前点加样式
            $('#'+ this.calendarId + '_'  + ymd).addClass((this.mOpts.theme == 'ta' ? this.mOpts.clickCss  : this.mOpts.theme + '_' + this.mOpts.selectCss));
            $('#'+ this.calendarId + '_'  + ymd).addClass(this.calendarId + '_'  + ymd);
            // 获取开始时间的初始值
            ...
            tmpEnd11111=$('#' + this.dateInput).val();
        } else if(this.endDateId == this.dateInput) {
            tmpBool=true;
            console.info("结束时间：",ymd);
            console.info($('#' + this.startDateId).val());
            ...
            tmpStart11111=$('#' + this.dateInput).val();
            if(ymdFormat == $('#' + this.startDateId).val()){
                showAlert({
                    msg:"入店和离店日期不能相同"
                });
                return false;
            ...
            this.removeCSS(0);
            // 为当前点加样式
            $('#'+ this.calendarId + '_'  + ymd).addClass((this.mOpts.theme == 'ta' ? this.mOpts.clickCss  : this.mOpts.theme + '_' + this.mOpts.compareCss));
            $('#'+ this.calendarId + '_'  + ymd).addClass(this.calendarId + '_'  + ymd);
            // 获取开始时间的初始值
            this.startDefDate = $('#' + this.dateInput).val();
            // 更改对应输入框的值
    ...
    pickerDateRange.prototype.close = function(btnSubmit) {
        ...
        if(btnSubmit){
            ...
            /************************************************************************************************/
            /************************************************************************************************/
            if($('#' + this.endDateId).val() == tmpEnd11111 && tmpBool==false){
                var end=msToDate(dateToMs($('#' + this.startDateId).val())+3600*24*1000);
                console.info($('#' + this.startDateId).val());
                console.info(this.mOpts.disCertainYMDay);
                if($.inArray($('#' + this.startDateId).val(),this.mOpts.disCertainYMDay)>=0){
                    showAlert({
                        msg:"当天满房，不能入住"
                    });
                    return false;
                }
                $('#' + this.endDateId).val(end);
            }else{
                tmpBool=false;
            }
            /************************************************************************************************/
            ...
            tmpStart=$('#' + this.startDateId).val();
            tmpEnd=$('#' + this.endDateId).val();
            /************************************************************************************************/
            /************************************************************************************************/
            /************************************************************************************************/
            /************************************************************************************************/
            ...
            $('#' + this.startDateId).val(tmpStart);
            $('#' + this.endDateId).val(tmpEnd);
        }
    ...
    pickerDateRange.prototype.fillDate = function(year, month, index) {
        ...
        for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
            if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
                ...
                //让固定日期不可选
                if(this.mOpts.disCertainYMDay && this.mOpts.disCertainYMDay.length > 0 ){
                    var isDisabled = false;
                    for(var p in this.mOpts.disCertainYMDay){
                        //if(!isNaN(this.mOpts.disCertainYMDay[p]) || isNaN(parseInt(this.mOpts.disCertainYMDay[p]))){
                            isDisabled = !!(d.toDateString() === new Date(this.mOpts.disCertainYMDay[p]).toDateString());
                            if ( isDisabled ){
                                break;
                            }
                        //}
                    ...
                    if ( isDisabled ){
                        tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray+' '+this.mOpts.theme + '_' + this.mOpts.fullRoomStyle;
                        deviation = '0';
                    }
                }
            }
    ...
    //lichunbo 封装
    $.pickerDateRange=function(args){
        var minDate = dateToMs(getNowDate())/1000;
        var maxDate = dateToMs(getNowDate())/1000 + 3600*24*90;
        var totalPrice = function(param){
            var sum=0;
            for(var i=0;i<args.datas.length;i++){
                if(new Date(args.datas[i].date).toJSON().substring(0,10)>=new Date(param.startDate).toJSON().substring(0,10)&&new Date(args.datas[i].date).toJSON().substring(0,10)<=new Date(param.endDate).toJSON().substring(0,10)){
                    sum+=args.datas[i].price;
                }
            }
            return sum;
        };
        var disCertainYMDay=[];
        for(var i=0;i<args.datas.length;i++){
            if(args.datas[i].isFull){
                disCertainYMDay.push(args.datas[i].date);
            }
        }
        var param={
                startDate: '',
                ...
                defaultText : ' 离开 ',
                autoSubmit : false,
                disCertainYMDay:disCertainYMDay,
                totalPrice:totalPrice,
                theme : 'ta'
            }
        $.extend(param,args);
        var dateRange1 = new pickerDateRange('date', param );

        for(var i=0;i<args.datas.length;i++){
            var date = new Date(args.datas[i].date);
            var dateId=dateRange1.calendarId+"_"+date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate();
            document.styleSheets[0].insertRule('.ta_calendar .'+dateId+'::after {content: "'+args.datas[i].price+'"}',1);
            document.styleSheets[0].addRule('.'+dateId+'::after','display: block');
            document.styleSheets[0].addRule('.'+dateId+'::after','font-size: 10px');
            document.styleSheets[0].addRule('.'+dateId+'::after','color: blue');//fff
        }
    }
