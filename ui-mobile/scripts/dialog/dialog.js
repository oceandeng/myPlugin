...
(function(G, D) {
    ...
    Dialog.prototype.init = function() {
        ...
        if (!F$(this.config_id) && !this.config_msg && !this.config_adviceVal && !this.config_selectItmes) {
            return false
        };
    ...
    Dialog.prototype.create = function() {
        ...
        switch(this.config_type){
            case "input":
                var html = "<div class='D_alert'><span>输入金额： </span><input id='adviceInp' data-relationId=" + this.config_relationId + " data-carAccId=" + this.config_carAccId + " style='border:1px solid #ccc; font-size: 16px; width: 40%; height:40px; line-height:40px; text-align:left; padding: 0 10px; background:#fff;' type='number' value = '" + this.config_adviceVal + "' />&nbsp;元</div>";
                body.innerHTML = html;
                ...
                this.foot = foot;
                this.fBtn = foot.querySelectorAll('input');
                this.subBtn = foot.querySelector('.D_submit');
                this.head = head;
            ...
            case "alert":
                ...
                content.className = "D_content";
                body.innerHTML = "<div class='D_alert'><div class='D_msg'>" + this.config_msg + "</div></div>";
                body.className = "D_body";
                ...
                this.head = head;
                this.bCon = body.querySelector('.D_alert');
                this.bMsg = this.bCon.querySelector('.D_msg');
                this.foot = foot;
                this.fBtn = foot.querySelectorAll('input');
                this.subBtn = foot.querySelector('.D_submit');
                this.closeBtn = D.querySelector('.D_close');
            ...
            case "select":
                ...
                var html = '<ul class="D_ul">';
                for(var i = 0, len = this.config_selectItmes.length; i < len; i++){
                    var item = this.config_selectItmes[i];

                    if(item.isActive){
                        html += '<li class="active" style="background: #fff0ed">' + this.config_selectItmes[i].name + '</li>'
                    }else{
                        html += '<li>' + this.config_selectItmes[i].name + '</li>'
                    }
                }
                html += '</ul>';

                body.innerHTML = "<div class='D_alert'>" + html + "</div>";
                body.className = "D_body";
                ...
                this.head = head;
                this.bCon = body.querySelector('.D_alert');
                this.dUl = this.bCon.querySelector('.D_ul');
                this.dLi = this.dUl.querySelectorAll('li');
                this.bMsg = this.bCon.querySelector('.D_msg');
                this.foot = foot;
                this.fBtn = foot.querySelectorAll('input');
                this.subBtn = foot.querySelector('.D_submit');
    ...
    Dialog.prototype.setStyle = function() {
        ...
        if(this.body){
            ...
            setStyle(this.bMsg, {
                ...
                'line-height': '60px',
                'padding': '0 0 0 60px',
                'background': 'url(' + this.config_icon + ') no-repeat left center',
    ...
    Dialog.prototype.event = function() {
        var elems = this.content.getElementsByTagName("input"),
            that = this;
