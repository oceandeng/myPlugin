...
var countDay, nowWeek;

//声明数据类型
var objPrices = {dataList:{}};	
	/*****************************************************************************
	/*****************************************************************************
	日期资料
	 *****************************************************************************/
	...
	//============================== 阴历属性
	function calElement(sYear, sMonth, sDay, week, lYear, lMonth, lDay, isLeap, cYear, cMonth, cDay,nowWeek,countDay,oldPrice,newPrice,only_id) {
		...
		this.oldPrice = oldPrice;//老价格
		this.newPrice = newPrice;//新价格
	 ...
	 */
	 //start
		var succ_datas ={
			 dataList:[
			 	{id:0,date:'2016-10-01',newPrice:'225',oldPrice:'564'},
			 	{id:1,date:'2016-10-02',newPrice:'225',oldPrice:'564'},
			 	{id:2,date:'2016-10-03',newPrice:'225',oldPrice:'564'},
			 	{id:3,date:'2016-10-04',newPrice:'225',oldPrice:'564'},
			 	{id:4,date:'2016-10-05',newPrice:'225',oldPrice:'564'},
			 	{id:5,date:'2016-10-06',newPrice:'225',oldPrice:'564'},
			 	{id:6,date:'2016-10-07',newPrice:'225',oldPrice:'564'},
			 	{id:7,date:'2016-10-08',newPrice:'225',oldPrice:'564'},
			 	{id:8,date:'2016-10-09',newPrice:'225',oldPrice:'564'},
			 	{id:9,date:'2016-10-10',newPrice:'225',oldPrice:'564'},
			 	{id:10,date:'2016-10-11',newPrice:'225',oldPrice:'564'},
			 	{id:11,date:'2016-10-12',newPrice:'225',oldPrice:'564'},
			 	{id:12,date:'2016-10-13',newPrice:'225',oldPrice:'564'},
			 	{id:13,date:'2016-10-14',newPrice:'225',oldPrice:'564'},
			 	{id:14,date:'2016-10-15',newPrice:'225',oldPrice:'564'},
			 	{id:15,date:'2016-10-16',newPrice:'225',oldPrice:'564'},
			 	{id:16,date:'2016-10-17',newPrice:'225',oldPrice:'564'},
			 	{id:17,date:'2016-10-18',newPrice:'225',oldPrice:'564'},
			 	{id:18,date:'2016-10-19',newPrice:'225',oldPrice:'564'},
			 	{id:19,date:'2016-10-20',newPrice:'225',oldPrice:'564'},
			 	{id:20,date:'2016-10-21',newPrice:'225',oldPrice:'564'},
			 	{id:21,date:'2016-10-22',newPrice:'225',oldPrice:'564'},
			 	{id:22,date:'2016-10-23',newPrice:'225',oldPrice:'564'},
			 	{id:23,date:'2016-10-24',newPrice:'225',oldPrice:'564'},
			 	{id:24,date:'2016-10-25',newPrice:'225',oldPrice:'564'},
			 	{id:25,date:'2016-10-26',newPrice:'225',oldPrice:'564'},
			 	{id:26,date:'2016-10-27',newPrice:'225',oldPrice:'564'},
			 	{id:27,date:'2016-10-28',newPrice:'225',oldPrice:'564'},
			 	{id:28,date:'2016-10-29',newPrice:'225',oldPrice:'564'},
			 	{id:29,date:'2016-10-30',newPrice:'225',oldPrice:'564'},
			 	{id:30,date:'2016-10-31',newPrice:'225',oldPrice:'564'}
			 ]
	}
	...
	function Calendar(y, m) {
		...
		countDay = this.length;
		this.firstWeek = sDObj.getDay(); //公历当月1日星期几
		nowWeek = this.firstWeek;
		...
		for (var i = 0; i < this.length; i++) {
			...
			this[i] = new calElement(y, m + 1, i + 1, nStr1[(i + this.firstWeek) % 7],
					lY, lM, lD++, lL,
					cY, cM, cD,nowWeek,countDay,succ_datas.dataList[i].newPrice,succ_datas.dataList[i].oldPrice,succ_datas.dataList[i].id);
			//end
		}
	...
	function init(target) {
		...
		$(target).find('.calendar-prevmonth').click(function () {
		     ...
		     var oMonth = oNow.getMonth()+2;
			 if(opts.year == oYear && opts.month<oMonth) return;
			 showMonth(target, -1);
		});
	...
	// 这是日历头部
     ...
     //$(target).find('.calendar-title span').html(yDisplay + '年 农历 ' + cyclical(opts.year - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】')	 
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				...
				if (opts.month == day[1]) {
					...
					//start
					if(info)
					dayHtml += '</span><br/><span class="newPrice" data-oldprice='+info.oldPrice+' id='+info.id+'>￥'+info.oldPrice+'</span><br/><span data-newprice='+info.newPrice+' class="oldPrice">￥'+info.newPrice+'</span>';
					if (opts.lunarDay) {
						dayHtml += "<br/>";
						if (opts.solarTerms && info.solarTerms) {
							//dayHtml += '<span color="#E0366E">' + info.solarTerms + '</span>';
						} else {
							if($.trim(info.lunarFestival)=="中秋节"){
								
							}
							if(info.lunarFestival){
								dayHtml += '<span size="2" style="font-size:9pt"></span><b><em>'+info.lunarFestival+'</em></b>';
							}else if(info.solarFestival){

								if($.trim(info.solarFestival)=="国庆"){
									dayHtml += '<span size="2" style="font-size:9pt" class="guoqing"></span><b><em>'+info.solarFestival+'</em></b>';
								}else{
									dayHtml += '<span size="2" style="font-size:9pt"></span><b><em>'+info.solarFestival+'</em></b>';
								}
							}
						}
					}
				...
				var day = $('<td class="calendar-day fullcalendar-day calendar-other-month"></td>').attr('abbr', day[0] + '-' + day[1] + '-' + day[2]).html(dayHtml).appendTo(tr);
     			if (info && (info.lunarFestival || info.solarFestival)) {
					day.addClass('festival');
				}
				day.click(function(e){
					if($(this).hasClass("calendar-other-month")) return;
				});
			}
		}
		var nowDay = currentCa;
		//start
	 	//t.find('td[abbr^="' + opts.year + '-' + opts.month + '"]').removeClass('calendar-other-month');
		var now = new Date();
		var today = now.getFullYear() + '-' + (now.getMonth() + 1) + '-' + now.getDate();
		var tEle = t.find('td[abbr^="' + opts.year + '-' + opts.month + '"]');
		tEle.each(function(k,v,i){
			
			if(tEle.eq(k).find("span").hasClass("guoqing")){
				tEle.slice(k, k+7).addClass("festival_bg");
			}
			if(v.abbr.split("-")[0]==now.getFullYear() && v.abbr.split("-")[2]>=now.getDate()){
				$(v).removeClass('calendar-other-month');
			}
			if(v.abbr.split("-")[0]==now.getFullYear() && v.abbr.split("-")[1]==now.getMonth()+1 && v.abbr.split("-")[2]<=now.getDate()){
				$(v).find("b").addClass('gray');
			}
			if(v.abbr.split("-")[0]==now.getFullYear() && v.abbr.split("-")[1]==now.getMonth()+1 && v.abbr.split("-")[2]<now.getDate()){
				$(v).find('.newPrice').hide();
				$(v).find('.oldPrice').hide();
			}
			if(v.abbr.split("-")[0]<now.getFullYear() || v.abbr.split("-")[1]<now.getMonth()+1){
				$(v).find('.newPrice').hide();
				$(v).find('.oldPrice').hide();
			}
			if(v.abbr.split("-")[0]==now.getFullYear() && v.abbr.split("-")[1]>now.getMonth()+1){
			...
			if(v.abbr.split("-")[0]>now.getFullYear())	$(v).removeClass('calendar-other-month');
			//if(opts.month)
			if(v.abbr.split("-")[0]<=now.getFullYear() && v.abbr.split("-")[1]<now.getMonth()+1){
				$(v).addClass('calendar-other-month');
				$(v).find("b").addClass('gray');
			}
		});
		...
		//市场价格
		$("#market").blur(function() {
			var _this = $(this);
			$(".calendar-body td").each(function(i) {

				if (!$(".calendar-body td").eq(i).hasClass("calendar-other-month")) {
					if (i % 7 < 5) {
						$(".calendar-body td").eq(i).find(".oldPrice").html('￥' + _this.val());
						var objs = {};
						objs.id = 112;
						objs.data = $(".calendar-body td").eq(i).attr('abbr');
						objs.oldPrice = _this.val();
						if (!objPrices.dataList[objs.data]) {
							objPrices.dataList[objs.data] = objs;
						} else {
							objPrices.dataList[objs.data]['oldPrice'] = _this.val();
						};
					}
				}
			});
			...
			$(".calendar-body td").each(function(i) {
				if (i % 7 == 5 || i % 7 == 6) {
					if (!$(".calendar-body td").eq(i).hasClass("calendar-other-month")) {
						...
						objs.id = 112;
						objs.data = $(".calendar-body td").eq(i).attr('abbr');
						$(".calendar-body td").eq(i).find(".newPrice").html('￥' + _this.val());
						objs.newPrice = _this.val();

						if (!objPrices.dataList[objs.data]) {
							objPrices.dataList[objs.data] = objs;
						} else {
							objPrices.dataList[objs.data]['newPrice'] = _this.val();
						};
					}
				}
			});
		...
		$('.save_btn').click(function() {
			$.each(objPrices.dataList, function(i, v) {
				 jsonData.dataList.push(v);
			});
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} '
		 + '.calendar-hover{border:1px solid #fb6e66;}.calendar-selected{border:1px solid #fb6e66;color: #333 !important;}.fullcalendar-body th{padding: 5px;height:20px;}'
